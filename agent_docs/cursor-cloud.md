...
- `bun run desktop:dev` first builds sidecar resources (`build:desktop-resources`), then runs `electron-vite dev`. D-Bus and GPU errors in logs are cosmetic on headless Linux and do not affect functionality. Dev builds expose CDP on `127.0.0.1:9322` by default for external UI automation or inspection (`COWORK_ELECTRON_REMOTE_DEBUG=0` disables it).
