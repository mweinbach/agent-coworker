...
The build pipeline rebuilds bundled desktop resources (`cowork-server` sidecar + prompts/config by default, with docs opt-in via `COWORK_BUNDLE_DESKTOP_DOCS=1`) via the root `build:desktop-resources` script. Codex app-server is resolved at runtime: Cowork uses an explicit `COWORK_CODEX_APP_SERVER_COMMAND`, then a system `codex app-server` that exposes the app-server subcommand, then a Cowork-managed install under `~/.cowork/codex-app-server`, and downloads the latest release from `openai/codex` when missing. Curated default skills are bootstrapped by the shared agent runtime into `~/.cowork/skills` from GitHub instead of being bundled into the app.
