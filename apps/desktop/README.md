...
The build pipeline rebuilds bundled desktop resources (`cowork-server` sidecar + prompts/config by default, with docs opt-in via `COWORK_BUNDLE_DESKTOP_DOCS=1`) via the root `build:desktop-resources` script. Codex app-server is resolved at runtime from the app-pinned Cowork-managed release under `~/.cowork/codex-app-server`; when the app bumps that code pin, the next runtime use downloads and promotes the replacement payload. Curated default skills are bootstrapped by the shared agent runtime into `~/.cowork/skills` from GitHub instead of being bundled into the app.
