...
Most release targets bundle a compiled `cowork-server-*` executable. Windows ARM64 packaging support remains in the build scripts, but the release workflow does not currently build or publish a Windows ARM64 desktop installer.
...
- packages the desktop app on macOS and Windows x64
...
Windows release artifact names are arch-specific. The published Windows desktop release is the x64 installer plus `latest.yml`; Windows ARM64 desktop installers and `latest-arm64.yml` are intentionally not published by this workflow.
