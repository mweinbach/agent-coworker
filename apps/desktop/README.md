...
The build pipeline rebuilds bundled desktop resources (`cowork-server` sidecar + prompts/config by default, with docs opt-in via `COWORK_BUNDLE_DESKTOP_DOCS=1`) via the root `build:desktop-resources` script. Codex app-server is resolved at runtime: Cowork uses an explicit `COWORK_CODEX_APP_SERVER_COMMAND`, then an existing Cowork-managed install under `~/.cowork/codex-app-server`, downloads the latest release from `openai/codex` when the managed install is missing, and falls back to a system `codex app-server` only if the managed install cannot be fetched and the system binary exposes the app-server subcommand. Curated default skills are bootstrapped by the shared agent runtime into `~/.cowork/skills` from GitHub instead of being bundled into the app.
