...
The build pipeline rebuilds bundled desktop resources (`cowork-server` sidecar + prompts/config by default, with docs opt-in via `COWORK_BUNDLE_DESKTOP_DOCS=1`) via the root `build:desktop-resources` script. Codex app-server is resolved at runtime: Cowork uses an explicit `COWORK_CODEX_APP_SERVER_COMMAND`, then a Cowork-managed install under `~/.cowork/codex-app-server`, then a system `codex app-server`, and downloads the latest release from `openai/codex` when missing. Set `COWORK_BUNDLE_CODEX_APP_SERVER=1` to bundle a build-time Codex app-server fallback, and optionally `COWORK_CODEX_APP_SERVER_VERSION` to pin that bundled fallback. Curated default skills are bootstrapped by the shared agent runtime into `~/.cowork/skills` from GitHub instead of being bundled into the app.
