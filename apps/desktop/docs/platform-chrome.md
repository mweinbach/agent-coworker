...
- `windowMaterial` — Windows-specific background material (e.g. `mica`).
...
- `win32.ts` — `titleBarOverlay` with transparent color, caption buttons, Mica.
...
- `AppTopBar` — orchestrates the top bar. Delegates left-side native chrome to
  `PlatformTopBarChrome` and uses normalized platform info for shared spacing.
...
- `win32.css` — Windows titlebar drag zones, caption reserve, titleband drag
  layer, chat-body offset.
- `linux.css` — Linux title bar padding and settings offsets.

Hardcoded pixel values (48px titlebar height, 10px drag strip height, etc.)
have been replaced with `var(--platform-*)` so the main process contract is
...
3. Expose it on `document.documentElement` from `App.tsx` and consume it via
   CSS variables or `useDesktopPlatform()`.
