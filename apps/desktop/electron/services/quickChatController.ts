...
type QuickChatControllerOptions = {
  ...
  createQuickChatWindow: () => Promise<BrowserWindow>;
};
...
export class QuickChatController {
  ...
  private readonly createQuickChatWindow: () => Promise<BrowserWindow>;
  ...
  async showQuickChatWindow(anchorBounds?: Rectangle): Promise<void> {
    const win = await this.ensureQuickChatWindow();
    this.positionQuickChatWindow(win, anchorBounds ?? this.tray?.getBounds());
    if (win.isMinimized()) {
      win.restore();
    }
    win.show();
  ...
  private ensureTray(): void {
    ...
    this.tray.on("click", () => {
      void this.toggleQuickChatWindow(this.tray?.getBounds());
    });
    ...
    return Menu.buildFromTemplate([
      {
        ...
        click: () => {
          void this.showQuickChatWindow(this.tray?.getBounds());
        },
      },
      {
  ...
  private async ensureQuickChatWindow(): Promise<BrowserWindow> {
    if (this.quickChatWindow && !this.quickChatWindow.isDestroyed()) {
  ...
  private positionQuickChatWindow(win: BrowserWindow, anchorBounds?: Rectangle): void {
