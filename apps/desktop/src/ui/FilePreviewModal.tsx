...
export function FilePreviewModal({
      ...
      try {
        ...
        if (previewKind === "docx") {
          ...
          const [htmlResult, layout] = await Promise.all([
            ...
            loadDocxPreviewLayout(arrayBuffer).catch(() => null),
          ]);
          if (!isCurrentLoad()) return;
  ...
  const content = (
    <>
      <DialogHeader
        className={cn(
          "shrink-0 flex flex-col gap-3 border-b app-border-subtle px-5",
          inline ? "py-3" : "py-4",
        )}
      >
        <div className="flex items-center justify-between gap-4 min-w-0">
          <div className="min-w-0 flex-1">
            <div className="flex min-w-0 items-center gap-2">
              {inline ? (
                <h2 className="truncate text-sm font-semibold text-foreground" title={titleName}>
                  {titleName}
                </h2>
              ) : (
                <DialogTitle
        ...
        {visibleTruncated ? (
          <div className="flex flex-wrap items-center justify-between gap-2 rounded-md border border-border bg-muted/40 px-3 py-2 text-xs text-muted-foreground">
            <span>
              Preview truncated — only the first portion of this file is shown. Open the file in its
              default app for the full contents.
            </span>
            <Button
              type="button"
              size="sm"
      ...
      <div
        ...
        className={cn(
          "min-h-0 flex-1",
          kind === "pdf" || kind === "xlsx" || kind === "csv" || kind === "pptx"
            ? "overflow-hidden p-0"
            : inline
              ? "overflow-y-auto px-6 py-8"
              : "overflow-y-auto px-5 py-4",
          kind === "docx" && visibleDocxHtml && "bg-background p-4 sm:p-6",
        )}
      >
        {visibleLoading ? (
          ...
          kind === "markdown" ? (
            <div data-file-preview-markdown-shell="true" className="mx-auto w-full max-w-[78ch]">
              <Streamdown
                ...
                className={cn(
                  "leading-7 [&>*:first-child]:mt-0 [&>*:last-child]:mb-0 [&_a]:underline [&_code]:rounded-sm [&_code]:bg-muted/45 [&_code]:px-1.5 [&_code]:py-0.5 [&_pre]:overflow-x-auto [&_pre]:rounded-md [&_pre]:border [&_pre]:app-border-subtle [&_pre]:bg-muted/35 [&_pre]:p-3",
                  inline &&
                    "text-sm [&_h1]:text-2xl [&_h1]:leading-tight [&_h2]:text-xl [&_h2]:leading-snug [&_h3]:text-lg [&_p]:leading-7",
                )}
          ...
          <div
            className={cn(
              "docx-preview mx-auto min-h-[11in] w-full max-w-[8.5in] rounded-sm border app-border-subtle bg-card px-6 py-8 text-foreground shadow-sm sm:px-10 sm:py-12",
          ...
          <InlineErrorBoundary label="This spreadsheet couldn't be previewed.">
            <LazyUniverSpreadsheetCanvas key={path} path={path} compact />
          </InlineErrorBoundary>
        ) : kind === "pptx" && path ? (
  ...
  if (inline) {
    ...
    return (
      <section
        aria-label={`${kindLabel} preview for ${titleName}`}
        className="app-file-preview-inline app-surface-opaque flex h-full min-h-0 w-full min-w-0 flex-col overflow-hidden"
