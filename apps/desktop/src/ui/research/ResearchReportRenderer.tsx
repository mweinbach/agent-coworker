...
export function ResearchReportRenderer({
  ...
  return (
    <DesktopMarkdown
      ...
      className="research-report max-w-none text-[0.925rem] leading-7 [&>*:first-child]:mt-0 [&_h1]:mt-8 [&_h1]:text-2xl [&_h1]:font-semibold [&_h1]:tracking-tight [&_h2]:mt-7 [&_h2]:text-xl [&_h2]:font-semibold [&_h3]:mt-5 [&_h3]:text-lg [&_h3]:font-semibold [&_p]:my-3 [&_ul]:my-3 [&_ol]:my-3 [&_li]:my-1 [&_p_code]:rounded-sm [&_p_code]:bg-muted/45 [&_p_code]:px-1.5 [&_p_code]:py-0.5 [&_li_code]:rounded-sm [&_li_code]:bg-muted/45 [&_li_code]:px-1.5 [&_li_code]:py-0.5 [&_[data-streamdown=code-block]]:relative [&_[data-streamdown=code-block]]:my-4 [&_[data-streamdown=code-block]]:rounded-md [&_[data-streamdown=code-block]]:border [&_[data-streamdown=code-block]]:app-border-subtle [&_[data-streamdown=code-block]]:bg-muted/25 [&_[data-streamdown=code-block]]:gap-0 [&_[data-streamdown=code-block]]:p-0 [&_[data-streamdown=code-block]]:overflow-hidden [&_[data-streamdown=code-block-header]]:h-7 [&_[data-streamdown=code-block-header]]:px-3 [&_[data-streamdown=code-block-header]]:border-b [&_[data-streamdown=code-block-header]]:app-border-subtle [&_[data-streamdown=code-block-header]]:bg-muted/20 [&_[data-streamdown=code-block-header]_span]:ml-0 [&_[data-streamdown=code-block-header]_span]:text-xs [&_[data-streamdown=code-block-header]_span]:font-medium [&_[data-streamdown=code-block-header]_span]:tracking-wide [&_[data-streamdown=code-block]>div:not([data-streamdown])]:absolute [&_[data-streamdown=code-block]>div:not([data-streamdown])]:top-0.5 [&_[data-streamdown=code-block]>div:not([data-streamdown])]:right-1.5 [&_[data-streamdown=code-block]>div:not([data-streamdown])]:mt-0 [&_[data-streamdown=code-block]>div:not([data-streamdown])]:h-6 [&_[data-streamdown=code-block]>div:not([data-streamdown])]:z-10 [&_[data-streamdown=code-block-actions]]:h-6 [&_[data-streamdown=code-block-actions]]:app-border-subtle [&_[data-streamdown=code-block-actions]]:bg-background/70 [&_[data-streamdown=code-block-body]]:border-0 [&_[data-streamdown=code-block-body]]:bg-transparent [&_[data-streamdown=code-block-body]]:rounded-none [&_[data-streamdown=code-block-body]]:px-4 [&_[data-streamdown=code-block-body]]:py-3 [&_pre]:bg-transparent [&_pre]:border-0 [&_pre]:p-0 [&_pre]:m-0"
    >
      {prepared}
    </DesktopMarkdown>
  );
