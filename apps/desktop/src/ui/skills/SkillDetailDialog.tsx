...
export function SkillDetailDialog({ workspaceId }: { workspaceId: string }) {
  ...
  const isOpen =
    !isDismissedAfterDelete && (selectedSkillInstallationId !== null || selectedSkillName !== null);
  ...
  const updateCheck = selectedInstallation
    ? (rt?.skillUpdateChecksByInstallationId[selectedInstallation.installationId] ?? null)
    : null;
                    ...
                    <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-muted/50 border border-border/50 text-2xl overflow-hidden">
                      <SkillIcon
                        icon={
                          ...
                          selectedInstallation?.interface?.iconSmall ||
                          selectedSkill?.interface?.iconLarge ||
                          selectedSkill?.interface?.iconSmall ||
                          "📦"
                        }
                      />
                    </div>
                      ...
                      <div className="flex items-center gap-2 text-sm text-muted-foreground">
                        <span>
                          {selectedInstallation
                            ? scopeLabel(selectedInstallation.scope)
                            : selectedSkill
              ...
              {selectedInstallation && (
                <div className="flex flex-wrap gap-2">
                  ...
                  <Badge variant="outline">
                    {selectedInstallation.writable ? "Writable" : "Read-only"}
                  </Badge>
                  ...
                  {selectedInstallation.plugin ? (
                    <Badge variant="outline">Plugin-owned</Badge>
                  ) : null}
                </div>
              )}
                ...
                {selectedInstallation ? (
                  selectedInstallation.enabled ? (
                    <Button
                      ...
                      disabled={
                        mutationBlocked ||
                        actionPending(rt, "disable", selectedInstallation.installationId)
                      }
                      onClick={() =>
                        void disableSkillInstallation(selectedInstallation.installationId)
                      }
                    >
                      Disable
                    </Button>
                    ...
                    <Button
                      ...
                      disabled={
                        mutationBlocked ||
                        actionPending(rt, "enable", selectedInstallation.installationId)
                      }
                      onClick={() =>
                        void enableSkillInstallation(selectedInstallation.installationId)
                      }
                    >
                      Enable
                    </Button>
                  ...
                  <>
                    ...
                    <Button
                      ...
                      disabled={
                        ...
                        actionPending(rt, "update", selectedInstallation.installationId)
                      }
                      onClick={() =>
                        void updateSkillInstallation(selectedInstallation.installationId)
                      }
                    >
                      Update
                    </Button>
