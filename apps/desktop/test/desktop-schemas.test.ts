...
describe("desktop persisted-state schema defaults", () => {
  ...
  test("accepts mobile relay bridge state payloads", () => {
    const parsed = mobileRelayBridgeStateSchema.parse({
      ...
      workspacePath: "/tmp/workspace",
      relaySource: "managed",
      relaySourceMessage: "Remodex was not found; using Cowork-managed relay state.",
      relayServiceStatus: "unknown",
      relayServiceMessage: "Cowork-managed mode does not use Remodex daemon health checks.",
