...
describe("desktop model stream mapper", () => {
  ...
  test("maps boundary and lifecycle parts", () => {
    ...
    expect(
      mapModelStreamChunk(chunk("start_step", { stepNumber: 2, request: { id: "rq1" } }, 2)),
    ).toEqual({
      kind: "step_start",
      turnId: "t1",
      stepNumber: 2,
    ...
    expect(
      mapModelStreamChunk(chunk("finish_step", { stepNumber: 2, response: { id: "rs1" } }, 3)),
    ).toEqual({
      kind: "step_finish",
      turnId: "t1",
      stepNumber: 2,
  ...
  test("maps text and reasoning stream parts", () => {
    ...
    expect(
      mapModelStreamChunk(
        chunk("reasoning_delta", { id: "r1", mode: "summary", text: "thinking" }, 4),
      ),
    ).toEqual({
  ...
  test("maps tool, source/file, and unknown parts", () => {
    expect(
      mapModelStreamChunk(
        chunk(
          "tool_input_start",
          { id: "tool_1", toolName: "read", args: { id: "tool_1", toolName: "read" } },
          0,
        ),
      ),
    ).toEqual({
    ...
    expect(
      mapModelStreamChunk(chunk("tool_input_delta", { id: "tool_1", delta: "abc" }, 1)),
    ).toEqual({
    ...
    expect(
      mapModelStreamChunk(chunk("tool_input_end", { id: "tool_1", toolName: "read" }, 2)),
    ).toEqual({
      kind: "tool_input_end",
      turnId: "t1",
      key: "tool_1",
    ...
    expect(
      mapModelStreamChunk(
        chunk(
          "tool_call",
          { toolCallId: "tool_1", toolName: "read", input: { path: "README.md" } },
          3,
        ),
      ),
    ).toEqual({
    ...
    expect(
      mapModelStreamChunk(
        chunk("tool_result", { toolCallId: "tool_1", toolName: "read", output: { chars: 42 } }, 4),
      ),
    ).toEqual({
      kind: "tool_result",
      turnId: "t1",
    ...
    expect(
      mapModelStreamChunk(
        chunk("tool_error", { toolCallId: "tool_1", toolName: "read", error: "nope" }, 5),
      ),
    ).toEqual({
      kind: "tool_error",
      turnId: "t1",
    ...
    expect(
      mapModelStreamChunk(
        chunk(
          "tool_output_denied",
          { toolCallId: "tool_1", toolName: "read", reason: "blocked" },
          6,
        ),
      ),
    ).toEqual({
    ...
    expect(
      mapModelStreamChunk(
        chunk("tool_approval_request", { approvalId: "ap-1", toolCall: { toolName: "bash" } }, 7),
      ),
    ).toEqual({
    ...
    expect(
      mapModelStreamChunk(
        chunk("source", { source: { kind: "url", value: "https://example.com" } }, 8),
      ),
    ).toEqual({
      kind: "source",
      turnId: "t1",
    ...
    expect(
      mapModelStreamChunk(
        chunk(
          "raw",
          {
            raw: { type: "response.function_call_arguments.delta", item_id: "fc_1", delta: "arg" },
          },
          11,
        ),
      ),
    ...
    expect(
      mapModelStreamChunk(
        chunk(
          "raw",
          { raw: { type: "response.output_text.delta", item_id: "txt_9", delta: "hello" } },
          11,
        ),
      ),
    ).toEqual({
    ...
    expect(
      mapModelStreamChunk(chunk("future_part_type", { experimental: true }, 14) as any),
    ).toEqual({
      kind: "unknown",
      turnId: "t1",
