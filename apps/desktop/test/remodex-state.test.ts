...
const { deriveRelayPublicKey, generateRelayKeyPair, RELAY_PAIRING_QR_VERSION } = await import(
  "../../../src/shared/mobileRelaySecurity"
);
const {
...
describe("remodex state reader", () => {
  ...
  test("reads valid remodex state and extracts the trusted phone", async () => {
    ...
    expect(state.identityState.trustedPhone?.phoneIdentityPublicKey).toBe(
      fixture.phone1KeyPair.publicKeyBase64,
    );
  ...
  test("repairs a mismatched remodex mac public key from the private key", async () => {
    ...
    await fs.writeFile(
      path.join(tmpDir, "device-state.json"),
      JSON.stringify(
        {
          version: 1,
          macDeviceId: "mac-1",
    ...
    expect(state.identityState.macIdentityPublicKey).toBe(
      deriveRelayPublicKey(fixture.macKeyPair.privateKeyBase64),
    );
    const repairedState = JSON.parse(
      await fs.readFile(path.join(tmpDir, "device-state.json"), "utf8"),
    );
    expect(repairedState.macIdentityPublicKey).toBe(
      deriveRelayPublicKey(fixture.macKeyPair.privateKeyBase64),
    );
  });
  ...
  test("persists trusted-phone updates back into remodex device state", async () => {
    ...
    const rawDeviceState = JSON.parse(
      await fs.readFile(path.join(tmpDir, "device-state.json"), "utf8"),
    );
...
async function writeRemodexState(
  ...
  await fs.writeFile(
    path.join(stateDir, "bridge-status.json"),
    JSON.stringify(
      {
        state: "running",
  ...
  await fs.writeFile(
    path.join(stateDir, "device-state.json"),
    JSON.stringify(
      {
        version: 1,
        macDeviceId: "mac-1",
  ...
  await fs.writeFile(
    path.join(stateDir, "pairing-session.json"),
    JSON.stringify(
      {
