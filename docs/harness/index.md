...
Research is ordinary harness execution. Users can ask research questions in chat, promote well-scoped work into Task mode, spawn helper agents, or run the bundled provider-agnostic `workflows/deep-research.ts` workflow. There is no separate `ResearchService`, analyst tab, research database, or research-specific protocol path in the active architecture.
...
- `reliability.md`: product reliability guarantees, failure boundaries, recovery invariants, and regression coverage.
- `ux-reliability-map.md`: bidirectional user-journey ownership, client-visible truth, and regression responsibilities.
...
- `docs/session-storage-architecture.md` for shared SQLite session and task persistence.
