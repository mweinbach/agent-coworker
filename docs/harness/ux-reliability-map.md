...
| Starting up does not make a usable chat unavailable. | New-chat composer, startup status, and Send button. | Workspace startup readiness and server-owned turn admission. | A pending startup that can accept a submission says the chat will start automatically; an actually blocked provider or workspace explains the corrective action. |
...
| I can see when the agent needs me. | Desktop/mobile approval cards, thread-list badges, and accessible announcements. | Durable interaction state, thread summaries, startup reconciliation, and `serverRequest/resolved`. | Pending approval or question state is discoverable without opening every conversation; cancellation, timeout, process interruption, or another device's decision clears it everywhere. |
| No always means no. | Approval sheet, confirmation controls, and cross-device resolution. | Authoritative prompt resolver, validated approval response, and durable interaction receipt. | Explicit rejection wins over contradictory legacy response fields; a missing runtime returns failure instead of pretending a prompt was resolved. |
...
| A failed sign-in lets me try again. | Provider API-key forms, authorization-code forms, and chat readiness. | Authoritative provider authentication acknowledgments and authorization state. | Rejected credentials and authorization codes remain in the current form for correction; errors are actionable, and an explicitly unauthorized provider cannot present a guaranteed-to-fail send as ready. |
...
8. If its owning process disappeared, can this supposedly pending operation
   still be resolved, or must recovery settle and clear it?
