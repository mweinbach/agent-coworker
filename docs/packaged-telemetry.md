...
Packaged releases must be safe by default. A public build may contain public project keys, but Cowork still must not initialize Sentry, PostHog, Langfuse, diagnostics upload, or cloud sync unless resolver policy allows it.
...
| `packaged-public` | Public signed desktop release. | Network telemetry and cloud sync off until user toggle plus matching public config exists. |
| `self-hosted` | Internal or customer-controlled deployment. | Runtime env/config may point to self-hosted endpoints. |
| `enterprise/offline` | Locked-down or offline deployment. | `COWORK_DISABLE_NETWORK_TELEMETRY=1` forces all network telemetry and sync off. |
...
| `COWORK_DIAGNOSTICS_UPLOAD_URL` | Yes | Optional support endpoint. |
| `COWORK_CLOUD_SYNC_ENDPOINT` | Yes | Optional self-hosted sync endpoint. |
...
The renderer and preload receive only safe resolver output. `LANGFUSE_SECRET_KEY` and cloud sync tokens are stripped from renderer-safe config even when present in server or main-process runtime env.
...
| Diagnostics upload | Local bundle creation works. Upload disabled unless Diagnostic log uploads is on, `COWORK_DIAGNOSTICS_UPLOAD_URL` exists, and the user confirms upload. |
| Cloud sync | Disabled unless the hidden cloud sync config is enabled and `COWORK_CLOUD_SYNC_ENDPOINT` exists. |
...
- Packaged public builds default all network telemetry and cloud sync off.
- User toggles are required before Sentry, PostHog, Langfuse, or diagnostics upload can start.
- `COWORK_DISABLE_NETWORK_TELEMETRY=1` disables Sentry, PostHog, Langfuse, diagnostics upload, and cloud sync globally.
- Local logs and local diagnostics bundle creation remain available while the kill switch is active.
- Self-hosted deployments can configure `COWORK_POSTHOG_HOST`, `LANGFUSE_BASE_URL`, `COWORK_DIAGNOSTICS_UPLOAD_URL`, and `COWORK_CLOUD_SYNC_ENDPOINT`.
