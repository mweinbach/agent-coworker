...
`cowork/workspace/presentation/preview` resolves its requested slide module or PowerPoint deck inside the server-owned workspace boundary. Successful results are `{ ok: true, path, slides, version }`, where `path` is the canonical resolved source path and `version` is the generic `{ modifiedAtMs, changeTimeMs, size, fingerprint }` file identity used by clients to coalesce and invalidate preview resources.

`cowork/workspace/fileChanged` is a generic invalidation notification emitted after successful Canvas document saves and spreadsheet patches. Params are `{ cwd, kind: "changed", path, version }` for a changed file or `{ cwd, kind: "deleted", path, version: null }` for a deleted file. Clients should invalidate resources whose requested or canonical path matches `path`; receiving the same version again is a no-op.
...
- `cowork/workspace/fileChanged` — params `{ cwd, kind, path, version }`; invalidates versioned file resources after workspace mutations
