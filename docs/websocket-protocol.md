...
- `task/updateGraph` — params `{ cwd?, taskId, expectedRevision, workItems }`; reconciles the validated dependency graph by stable work-item IDs and returns `{ task }`; compatible updates preserve ownership, evidence, artifact links, thread links, and active artifact revision rows, while removing or rekeying active revision work rejects atomically
