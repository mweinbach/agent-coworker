...
export function expandCommandTemplate(template: string, argumentsText: string): string {
  ...
  // Replace only markers in the original template. Callback replacements keep
  // dollar signs and placeholder-like text in the user's arguments literal.
  let output = template.replace(/\$ARGUMENTS|\$(\d+)/g, (match, indexRaw: string | undefined) => {
    if (match === "$ARGUMENTS") return trimmedArguments;
