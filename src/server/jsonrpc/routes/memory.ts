...
type AdvancedMemoryFolderMode = "current" | "explicit";

export function createMemoryRouteHandlers(context: JsonRpcRouteContext): JsonRpcRequestHandlerMap {
  const resolveAdvancedMemoryFolder = (
    ws: Parameters<JsonRpcRequestHandler>[0],
    message: Parameters<JsonRpcRequestHandler>[1],
    params: Record<string, unknown>,
    mode: AdvancedMemoryFolderMode,
  ): string | undefined | null => {
    if (mode === "current") {
      if (Object.hasOwn(params, "folder")) {
        context.jsonrpc.sendError(ws, message.id, {
          code: JSONRPC_ERROR_CODES.invalidParams,
          message: `${message.method}: folder is not accepted; use cowork/memory/advanced/folder/* for explicit folder access`,
        });
        return null;
      }
      return undefined;
    ...
    if (!folder) {
      context.jsonrpc.sendError(ws, message.id, {
        code: JSONRPC_ERROR_CODES.invalidParams,
        message: `${message.method}: folder is required`,
      });
      return null;
    }
    return folder;
  ...
  const handleAdvancedList =
    (mode: AdvancedMemoryFolderMode): JsonRpcRequestHandler =>
    async (ws, message) => {
      const params = toJsonRpcParams(message.params);
      const folder = resolveAdvancedMemoryFolder(ws, message, params, mode);
  ...
  return {
    ...
    "cowork/memory/advanced/list": handleAdvancedList("current"),
    "cowork/memory/advanced/upsert": handleAdvancedUpsert("current"),
    "cowork/memory/advanced/delete": handleAdvancedDelete("current"),
    "cowork/memory/advanced/generate": handleAdvancedGenerate("current"),
    "cowork/memory/advanced/folder/list": handleAdvancedList("explicit"),
    "cowork/memory/advanced/folder/upsert": handleAdvancedUpsert("explicit"),
    "cowork/memory/advanced/folder/delete": handleAdvancedDelete("explicit"),
    "cowork/memory/advanced/folder/generate": handleAdvancedGenerate("explicit"),
  };
}
