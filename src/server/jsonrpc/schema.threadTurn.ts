...
const inputPart = z.discriminatedUnion("type", [
  ...
  legacyInputTextPart,
  fileInputPart,
  uploadedFileInputPart,
]);
const turnInputPartsSchema = z.array(inputPart).superRefine((input, ctx) => {
  const attachments = input.filter(
    (part): part is z.infer<typeof fileInputPart> | z.infer<typeof uploadedFileInputPart> =>
  ...
  const inlineAttachments = attachments.filter(
    (part): part is z.infer<typeof fileInputPart> => part.type === "file",
  );
