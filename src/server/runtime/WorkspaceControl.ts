...
import { resolveWorkspaceA2ui } from "./ConfigPatchStore";

export type WorkspaceControlRefreshEvent = Extract<
  ServerEvent,
  { type: "skills_list" | "skills_catalog" | "plugins_catalog" | "mcp_servers" }
>;

type WorkspaceControlOptions = {
  yolo?: boolean;
  sendJsonRpc(ws: StartServerSocket, payload: unknown): void;
  shouldSendJsonRpcNotification(ws: StartServerSocket, method: string): boolean;
  buildSession(
  ...
  loadWorkspaceControlConfig(cwd: string): Promise<AgentConfig>;
  disposeBinding(binding: SessionBinding, reason: string): void;
  captureEvent<T extends ServerEvent>(
    binding: SessionBinding,
    action: () => Promise<void>,
...
export class WorkspaceControl {
  ...
  constructor(private readonly options: WorkspaceControlOptions) {}
  ...
  async getOrCreateBinding(cwd: string): Promise<SessionBinding> {
    ...
    const built = this.options.buildSession(binding, undefined, {
      config: await this.options.loadWorkspaceControlConfig(cwd),
  ...
  async withSession<T>(
    ...
    try {
      ...
      this.options.disposeBinding(binding, `workspace control request completed for ${cwd}`);
    }
  }

  async readState(cwd: string): Promise<ServerEvent[]> {
    return this.buildStateEventsFromConfig(await this.options.loadWorkspaceControlConfig(cwd));
  }
  ...
  private notifySubscribers(cwd: string, event: WorkspaceControlRefreshEvent): void {
    ...
    for (const ws of subscribers.values()) {
      if (!this.options.shouldSendJsonRpcNotification(ws, "cowork/control/event")) {
        continue;
      }
      this.options.sendJsonRpc(ws, {
  ...
  private async readRefreshEvents(cwd: string): Promise<WorkspaceControlRefreshEvent[]> {
    ...
    const captureRefreshEvent = async <T extends ServerEvent>(
      ...
      try {
        return await this.options.captureEvent(binding, action, predicate);
      } catch {
        return null;
      }
    };
