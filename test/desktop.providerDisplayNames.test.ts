...
describe("desktop provider fallback auth methods", () => {
  test("keeps Bedrock structured fallback fields intact", () => {
    ...
    expect(methods.find((method) => method.id === "aws_default")?.fields?.map((field) => field.id)).toEqual(["region"]);
    expect(methods.find((method) => method.id === "aws_profile")?.fields?.map((field) => field.id)).toEqual([
      "profile",
      "region",
    ]);
    expect(methods.find((method) => method.id === "aws_keys")?.fields?.map((field) => field.id)).toEqual([
      "accessKeyId",
      "secretAccessKey",
      "sessionToken",
      "region",
    ]);
    expect(methods.find((method) => method.id === "api_key")?.fields?.map((field) => field.id)).toEqual([
      "apiKey",
      "region",
    ]);
  });
