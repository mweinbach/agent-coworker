...
describe("server JSON-RPC explicit tool retries", () => {
  test("confirms exact lineage, scopes reused provider ids, persists restart state, and rolls back for old clients", {
    ...
    const retryClientMessageId = "retry-message-exact-once";
    const retryRequest = () => ({
      threadId,
...

    {
      ...
      try {
        ...
        const invalidRetry = await rpc.sendRequest("turn/start", {
          ...retryRequest(),
          retry: { toolItemIds: ["missing-tool-item"] },
        });
        expect(invalidRetry.error?.message).toContain("not an unresolved failed tool");

        const retryResponse = await rpc.sendRequest("turn/start", retryRequest());
        ...
        await rpc.waitFor((message) => message.method === "turn/completed");
        const replayedRetry = await rpc.sendRequest("turn/start", retryRequest());
        expect(replayedRetry.result).toMatchObject({
          replayed: true,
          turn: { id: retryResponse.result.turn.id },
        });
        expect(invocation).toBe(2);

        const conflictingRetry = await rpc.sendRequest("turn/start", {
          ...retryRequest(),
          retry: { toolItemIds: [replacementItemId] },
        });
        expect(conflictingRetry.error?.message).toContain("clientMessageId conflict");
        expect(invocation).toBe(2);
    ...
    try {
      const rpc = await connectJsonRpc(url, { toolRetryLineage: true });
      const replayedRetry = await rpc.sendRequest("turn/start", retryRequest());
      expect(replayedRetry.result.replayed).toBe(true);
      expect(invocation).toBe(2);
      const read = await rpc.sendRequest("thread/read", { threadId });
      expect(toolItemsFromSnapshot(read.result.coworkSnapshot)).toEqual(
        expect.arrayContaining([
