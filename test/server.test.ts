...
describe("WebSocket Lifecycle", () => {
  test("on connect, receives server_hello with sessionId", async () => {
    ...
    try {
      ...
      // Legacy protocol version check removed — JSON-RPC only now
    } finally {
      server.stop();
    }
  });
...
// NOTE: Legacy "Protocol Doc Parity" tests removed — they tested the legacy
// WebSocket protocol which has been replaced by JSON-RPC. See
// test/server.jsonrpc.test.ts and test/server.jsonrpc.flow.test.ts for
