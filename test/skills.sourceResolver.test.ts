...
import {
  ...
  resolveSkillSource,
} from "../src/skills/sourceResolver";
...
describe("resolveSkillSource", () => {
  ...
  test("prefers an existing relative local path over owner/repo shorthand", async () => {
    ...
    try {
      ...
      await fs.writeFile(
        path.join(absoluteSkill, "SKILL.md"),
        skillDoc("my-skill", "Local skill."),
        "utf-8",
      );
...
describe("local source materialization and preview", () => {
  test("materializes a local skill directory and previews install impact", async () => {
    ...
    await fs.writeFile(
      path.join(localSkills, "alpha", "SKILL.md"),
      skillDoc("alpha", "Alpha skill."),
      "utf-8",
    );
